> Get a direct overview of Pull Request difficulty.
...
The paper describing the research behind this extension is available at: [PRECOG paper](https://researchportal.unamur.be/en/publications/precog-pull-request-prioritization-and-visualization/).

# Purpose

The main goal of the extension is to provide reviewers with a **quick overview of Pull Request difficulty**.

Recent research has shown that **cognitive biases** can influence how developers review Pull Requests, which may negatively impact review quality. By offering a visual summary of a Pull Request’s difficulty, this extension aims to:

- Raise awareness about difficulty before review
...
1. `Fit For Me` Pull Request List

   Pull Requests are automatically sorted into a new list called **"Fit For Me"**, based on their **total difficulty score**. This score is determined using an aggregate formula based on a set of metrics describing the difficulty of the Pull Request.

   ![Fit For Me Category](.readme/fitForMe.png)

2. Radar Chart Visualization

   Each Pull Request overview includes a **radar chart** displaying various metrics on the Pull Request.
   ...
   - The chart gives reviewers an immediate, visual understanding of the Pull Request’s difficulty profile.

   ![Radar Chart](.readme/radarChart.png)

# Getting Started

This extension relies on a **backend API** that analyzes Pull Requests and computes difficulty metrics.
...
2. **Configure the analysis server URL**
   In the extension settings, locate the field called **`PRECOG: Server URL`** and enter the URL of your analysis server.

You're now ready to go! 🎉

# Configuring the Extension

The configuration of difficulty metrics is handled by the **analysis server**.
...
All settings added by this extension are labeled with **"PRECOG"** in the Visual Studio Code settings panel.

# Architecture of the Extension

The functionality added by this extension is implemented in the `src/pullRequestWithMetrics/` folder.
...
- **`pullRequesWithMetricsClient`**
  A client module responsible for communicating with the analysis server and retrieving difficulty metrics for Pull Requests.
...
- **`sorteablePullRequests`**
  A helper object that retrieves metrics for multiple Pull Requests and sorts them based on their **worst difficulty value**.
